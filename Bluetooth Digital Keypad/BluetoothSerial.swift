...
//            if let r = defaults.string(forKey: "record"){
//                record = r
//            }
            Console.textRecord += " \(time.hour ?? 00):\(time.minute ?? 00):\(time.second ?? 00) Write: \(message) \n"
            
//            UserDefaults.standard.set(message, forKey: "record")
            print(record)
        }
    }
